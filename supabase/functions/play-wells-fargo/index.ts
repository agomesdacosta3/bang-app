...
serve(async (req) => {
  ...
  try {
    ...
    const { data: card } = await supabaseAdmin.from('hand_cards').select('id, suit, value').eq('player_id', me.id).eq('card_type', 'wells_fargo').limit(1).single();
    if (!card) throw new Error('Vous n’avez pas de carte Convoi en main');

    await supabaseAdmin.from('hand_cards').delete().eq('id', card.id);
    await supabaseAdmin.from('discard_pile').insert({ game_id: gameId, card_type: 'wells_fargo', suit: card.suit, value: card.value });
