...
serve(async (req) => {
  ...
  try {
    ...
    if (handCardId) {
      ...
      const { data: eq } = await supabaseAdmin.from('cards_in_play').select('id').eq('player_id', me!.id).eq('card_type', inPlayCardType).maybeSingle();
      ...
      await supabaseAdmin.from('discard_pile').insert({ game_id: gameId, card_type: inPlayCardType, suit: rv.suit, value: rv.value });
      discardedType = inPlayCardType;
    } else {
      throw new Error('Indiquez une carte à défausser');
    }

    await logEvent(gameId, 'card_discarded_forced', { actorSeat: me!.seat_position, cardType: discardedType, threadId });
    if (handCardId) await checkSuzyLafayette(gameId, me!.id, threadId);
