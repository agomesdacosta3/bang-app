...
serve(async (req) => {
  ...
  try {
    ...
    const eventId = await logEvent(gameId, 'indians_played', { actorSeat: me.seat_position });
    const others = players!.filter(p => p.is_alive && p.id !== me.id);
    await startPending(gameId, me.id, 'indians_response', others.map(p => ({ playerId: p.id, isCurrentTurn: true })), 20000, eventId);
