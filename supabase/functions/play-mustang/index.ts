...
serve(async (req) => {
  ...
  try {
    ...
    const { data: existing } = await supabaseAdmin.from('cards_in_play').select('id').eq('player_id', me.id).eq('card_type', 'mustang').maybeSingle();
    if (existing) throw new Error('Vous avez déjà un Mustang en jeu');

    const { data: card } = await supabaseAdmin.from('hand_cards').select('id, suit, value').eq('player_id', me.id).eq('card_type', 'mustang').limit(1).single();
    if (!card) throw new Error('Vous n’avez pas de carte Mustang en main');

    await supabaseAdmin.from('hand_cards').delete().eq('id', card.id);
    await supabaseAdmin.from('cards_in_play').insert({ player_id: me.id, card_type: 'mustang', suit: card.suit, value: card.value });
    await logEvent(gameId, 'mustang_equipped', { actorSeat: me.seat_position });
