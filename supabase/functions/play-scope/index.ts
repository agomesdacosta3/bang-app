...
serve(async (req) => {
  ...
  try {
    ...
    const { data: existing } = await supabaseAdmin.from('cards_in_play').select('id').eq('player_id', me.id).eq('card_type', 'scope').maybeSingle();
    if (existing) throw new Error('Vous avez déjà une Lunette en jeu');

    const { data: card } = await supabaseAdmin.from('hand_cards').select('id, suit, value').eq('player_id', me.id).eq('card_type', 'scope').limit(1).single();
    ...
    await logEvent(gameId, 'scope_equipped', { actorSeat: me.seat_position });
