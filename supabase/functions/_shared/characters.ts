...
// Suzy Lafayette : pioche 1 carte dès que sa main devient vide. À appeler après toute suppression de carte(s) en main.
export async function checkSuzyLafayette(gameId: string, playerId: string) {
  ...
  if (hand && hand.length === 0) {
    ...
    await logEvent(gameId, 'suzy_lafayette_draw', { actorSeat: player.seat_position });
  }
}