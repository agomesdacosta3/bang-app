...
serve(async (req) => {
  ...
  try {
    ...
    if (character !== 'jesse_jones') throw new Error('Cette action est réservée à Jesse Jones');
    ...
    const { data: target } = await supabaseAdmin.from('players').select('*').eq('id', targetPlayerId).eq('game_id', gameId).maybeSingle();
    ...
    await checkSuzyLafayette(gameId, target.id);
    ...
    await logEvent(gameId, 'jesse_jones_steal', { actorSeat: me.seat_position, targetSeat: target.seat_position });
