...
serve(async (req) => {
  ...
  try {
    ...
    const { data: card } = await supabaseAdmin.from('hand_cards').select('id, suit, value').eq('player_id', me.id).eq('card_type', 'stagecoach').limit(1).single();
    if (!card) throw new Error('Vous n’avez pas de carte Diligence en main');

    await supabaseAdmin.from('hand_cards').delete().eq('id', card.id);
    await supabaseAdmin.from('discard_pile').insert({ game_id: gameId, card_type: 'stagecoach', suit: card.suit, value: card.value });
