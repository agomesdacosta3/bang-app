...
import { OfflineGameState, CardInstance } from '../offline/types';
import { WEAPON_TYPES, getWeaponRange } from '../lib/weapons';
import { colors, fonts, cardLabels, equipmentTags, roleLabels, roleObjectives, winnerTeamLabels, renderPips, suitLabels } from '../theme';
...
export default function OfflineGameScreen({ engine, onLeave }: { engine: OfflineEngine; onLeave: () => void }) {
  ...
  if (state.status === 'finished') {
    ...
    return (
      <View style={styles.centerContainer}>
        ...
        <Text style={styles.bodyText}>Vous étiez {roleLabels[me.role]}.</Text>
  ...
  const canTryBarrel = hasBarrelInPlay && (myPendingTarget?.barrelTriesUsed ?? 0) < 1;
  ...
  const canAct = isMyTurn && !hasPending && state.turnPhase === 'play' && !discarding;
  ...
  const canPlayBang = canAct && bangTargets.length > 0 && (hasVolcanic || !me.hasPlayedBangThisTurn);
  ...
  function nameFor(id: string | null | undefined): string {
    return state.players.find(p => p.id === id)?.nickname ?? 'Un joueur';
  }
  function nameForSeat(seat: number | null): string {
    return state.players.find(p => p.seatPosition === seat)?.nickname ?? 'Un joueur';
  }
  ...
  function handleTryBarrel() {
    try {
      state.pending!.type === 'gatling_response' ? engineRef.current.respondGatling(humanId, 'try_barrel') : engineRef.current.respondBang(humanId, 'try_barrel');
      const after = engineRef.current.getState();
      const stillPendingForMe = after.pending?.targets.find(t => t.playerId === humanId);
      if (stillPendingForMe && stillPendingForMe.barrelTriesUsed > 0) {
        // On ne sait pas directement si ça a marché sans relire le dernier événement — affiche l'événement le plus récent
        const lastEvent = after.events[after.events.length - 1];
        if (lastEvent.eventType === 'barrel_failed') Alert.alert('Planque ratée', `Carte tirée : ${suitLabels[lastEvent.drawnSuit ?? ''] ?? lastEvent.drawnSuit}. Choisissez une autre réponse.`);
      }
      refresh();
  ...
  return (
    <ScrollView style={styles.flexFill} contentContainerStyle={styles.container}>
      <View style={styles.plaque}>
        ...
        <Text style={styles.plaqueSub}>{me.nickname} · {myWeaponName} (portée {myWeaponRange})</Text>
      </View>
      ...
      <View style={styles.historyBox}>
        <ScrollView nestedScrollEnabled>
          {state.events.slice(-20).map(e => (
            <Text key={e.id} style={styles.noticeBody}>• {describeEvent(e)}</Text>
          ))}
        </ScrollView>
      </View>
        ...
        <ScrollView style={styles.confrontationScroll} nestedScrollEnabled>
          ...
          {mustChooseCatBalouDiscard && (
            <>
              ...
              {myHand.map(c => (
                <WoodButton key={c.id} title={`${cardLabels[c.type] ?? c.type} (main)`} onPress={() => runAction(() => engineRef.current.respondCatBalou(humanId, c.id))} style={styles.noticeBtn} />
              ))}
              {myEquipTypes.map(t => (
                <WoodButton key={t} title={`${cardLabels[t] ?? t} (en jeu)`} onPress={() => runAction(() => engineRef.current.respondCatBalou(humanId, undefined, t))} style={styles.noticeBtn} />
              ))}
            </>
          )}
          {isMyStoreTurn && (
            <>
              ...
              {state.generalStoreCards.map(c => (
                <WoodButton key={c.id} title={cardLabels[c.type] ?? c.type} onPress={() => runAction(() => engineRef.current.pickGeneralStoreCard(humanId, c.id))} style={styles.noticeBtn} />
              ))}
            </>
          )}
      ...
      {!discarding && (
        <View style={styles.handRow}>
          ...
          {myHand.map(item => {
            ...
            if (item.type === 'bang' && canPlayBang) { playable = true; onPress = () => setTargetPickerFor(item.id); }
      ...
      {canDraw && <WoodButton title="Piocher" onPress={() => runAction(() => engineRef.current.drawCards(humanId))} style={styles.fullWidthBtn} />}
      {canAct && (
        <WoodButton
      ...
      <Modal visible={!!targetPickerFor} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            {bangTargets.map(t => <WoodButton key={t.id} title={t.nickname} onPress={() => { setTargetPickerFor(null); runAction(() => engineRef.current.playBang(humanId, t.id)); }} style={styles.noticeBtn} />)}
      ...
      <Modal visible={!!duelTargetPickerFor} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            {duelTargets.map(t => <WoodButton key={t.id} title={t.nickname} onPress={() => { setDuelTargetPickerFor(null); runAction(() => engineRef.current.playDuel(humanId, t.id)); }} style={styles.noticeBtn} />)}
      ...
      <Modal visible={!!prisonTargetPickerFor} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            {prisonTargets.map(t => <WoodButton key={t.id} title={t.nickname} onPress={() => { setPrisonTargetPickerFor(null); runAction(() => engineRef.current.playPrison(humanId, t.id)); }} style={styles.noticeBtn} />)}
      ...
      <Modal visible={!!panicTargetPickerFor} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            {panicTargets.map(t => (
              <WoodButton key={t.id} title={t.nickname} onPress={() => { setPanicTargetPickerFor(null); setStealFlow({ targetId: t.id }); }} style={styles.noticeBtn} />
            ))}
      ...
      <Modal visible={!!catbalouTargetPickerFor} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            {catBalouTargets.map(t => <WoodButton key={t.id} title={t.nickname} onPress={() => { setCatbalouTargetPickerFor(null); runAction(() => engineRef.current.playCatBalou(humanId, t.id)); }} style={styles.noticeBtn} />)}
