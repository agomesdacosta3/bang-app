...
const ROLE_SETUP: Record<number, { outlaws: number; deputies: number }> = {
  4: { outlaws: 2, deputies: 0 },
  5: { outlaws: 2, deputies: 1 },
  6: { outlaws: 3, deputies: 1 },
  7: { outlaws: 3, deputies: 2 },
};
...
serve(async (req) => {
  ...
  try {
    ...
    // 1. Répartition des rôles (le Shérif est tiré au hasard, comme les autres rôles)
    ...
    // 3. Main initiale = autant de cartes que de points de vie
    ...
    // 4. Démarrage — premier tour pour le Shérif
